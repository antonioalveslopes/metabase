...
(deftest render-bar-graph-test
  ...
  (testing "Check if colors can be processed"
    (is (has-inline-image?
         (render-bar-graph {:cols default-columns
                            :rows [[10.0 1] [5.0 10] [nil 20] [1.25 nil]]
                            :viz-settings {:series_settings
                                           {:NumPurchased
                                            {:color "hsl(88.89999999999998, 47.1%, 65%)"}}}}))))
  ...
  (testing "Check if single x-axis combo series can convert colors"
    (is (= [{:name "NumPurchased", :color "#a7cf7b", :type :bar, :data [[1.25 20] [5.0 10] [10.0 1]], :yAxisPosition "left"}]
           (#'body/single-x-axis-combo-series
            ...
            {:series_settings {:NumPurchased {:color "hsl(88.89999999999998, 47.1%, 65%)"}}}))))
    ...
    (is (= [{:name "Bob", :color "#c5a9cf", :type "line", :data [[10.0 123]], :yAxisPosition "left"}
           ...
           (#'body/double-x-axis-combo-series
            ...
            {:series_settings {:Bob {:color "#c5a9cf"}
                               :Dobbs {:color "hsl(88.89999999999998, 47.1%, 65%)"}
                               :Robbs {:color "cmyk(58%, 35%, 0%, 51%)"}
                               :Mobbs {:color "rgb(224, 190, 64)"}}})))))
